...
class ImageVectorizer:
                ...
                if not file_info_exists:
                    ...
                    cursor.execute("""
                        CREATE TABLE SDS_FILES (
                            ...
                            CONSTRAINT PK_SDS_FILES PRIMARY KEY (FILE_ID)
                        ) DEFAULT COLLATION USING_NLS_COMP
                    """)
                    connection.commit()
                    logger.info("SDS_FILESテーブル作成完了")
                
                # SDS_IMAGE_EMBEDDINGSテーブルの存在確認
                cursor.execute("""
                    SELECT COUNT(*) 
                    FROM USER_TABLES 
                    WHERE TABLE_NAME = 'SDS_IMAGE_EMBEDDINGS'
                """)
                img_embeddings_exists = cursor.fetchone()[0] > 0
                
                if not img_embeddings_exists:
                    logger.info("SDS_IMAGE_EMBEDDINGSテーブルを作成します...")
                    cursor.execute("""
                        CREATE TABLE SDS_IMAGE_EMBEDDINGS (
                            ...
                            CONSTRAINT PK_SDS_IMAGE_EMBEDDINGS PRIMARY KEY (ID),
                            CONSTRAINT FK_SDS_IMAGE_FILE FOREIGN KEY (FILE_ID)
                                REFERENCES SDS_FILES(FILE_ID) ON DELETE CASCADE
                        ) DEFAULT COLLATION USING_NLS_COMP
                    """)
                    connection.commit()
                    logger.info("SDS_IMAGE_EMBEDDINGSテーブル作成完了")
    ...
    def _image_to_base64(self, image_data: io.BytesIO, content_type: str = "image/png") -> str:
        ...
        OCI Cohere Embed v4.0 はbase64画像が約5MBを超えると拒否するため、
        上限を超える画像はJPEGへ再エンコード＋縮小してから送信する。
        """
        image_data.seek(0)
        image_bytes = image_data.read()
        # base64は元バイトの約4/3。安全側で生バイト4MBを閾値にする。
        max_raw_bytes = int(os.environ.get("EMBEDDING_IMAGE_MAX_BYTES", str(4 * 1024 * 1024)))
        ...
        for _ in range(8):
            ...
            if len(data) <= max_raw_bytes:
                logger.info(f"画像を縮小: {len(image_bytes)}B -> {len(data)}B (size={img.size}, q={quality})")
                b64 = base64.b64encode(data).decode('utf-8')
                return f"data:image/jpeg;base64,{b64}"
            # まず解像度を7割に、品質も少しずつ落とす
            img = img.resize((max(1, int(img.width * 0.7)), max(1, int(img.height * 0.7))))
            quality = max(60, quality - 5)
        # フォールバック（最終縮小版を使用）
        b64 = base64.b64encode(data).decode('utf-8')
        logger.warning(f"画像縮小上限到達: 最終{len(data)}B")
        return f"data:image/jpeg;base64,{b64}"
    ...
    def save_image_embedding(self, file_id: int, bucket: str, object_name: str, 
        ...
        try:
            ...
            with self._get_pool_manager().acquire_connection() as connection:
                ...
                with connection.cursor() as cursor:
                    cursor.execute("""
                        INSERT INTO SDS_IMAGE_EMBEDDINGS
    ...
    def delete_file_embeddings(self, file_id: int) -> bool:
        ...
        try:
            ...
            with self._get_pool_manager().acquire_connection() as connection:
                with connection.cursor() as cursor:
                    cursor.execute("""
                        DELETE FROM SDS_IMAGE_EMBEDDINGS
    ...
    def get_vectorization_status(self, bucket: str, object_names: List[str]) -> Dict[str, bool]:
        ...
        try:
            ...
            with self._get_pool_manager().acquire_connection() as connection:
                with connection.cursor() as cursor:
                    cursor.execute(
                        "SELECT COUNT(*) FROM USER_TABLES WHERE TABLE_NAME='SDS_DOCUMENTS'"
                    )
                    if cursor.fetchone()[0]:
                        placeholders = ', '.join([f":rag_obj_{i}" for i in range(len(object_names))])
                        query = f"""
                            SELECT d.object_name,
                                   CASE WHEN EXISTS (
                                       SELECT 1 FROM sds_document_index_runs ir
                                       WHERE ir.document_id=d.document_id
                                         AND ir.is_serving=1 AND ir.status='SUCCEEDED'
                                   ) THEN 1 ELSE 0 END has_embeddings
                            FROM sds_documents d
                            WHERE d.bucket=:bucket AND d.object_name IN ({placeholders})
                        """
                        params = {'bucket': bucket}
                        params.update({f'rag_obj_{i}': name for i, name in enumerate(object_names)})
                        cursor.execute(query, params)
                        rows = cursor.fetchall()
                        for object_name, has_embeddings in rows:
                            result[object_name] = has_embeddings == 1
                        return result

                    # SDS_FILESテーブルとSDS_IMAGE_EMBEDDINGSテーブルを結合して、
                    ...
                    query = f"""
                        ...
                        FROM SDS_FILES f
                        LEFT JOIN (
                            SELECT DISTINCT FILE_ID FROM SDS_IMAGE_EMBEDDINGS
    ...
    def search_similar_images(self, query_embedding: np.ndarray, limit: int = 10, threshold: float = 0.7, filename_filter: Optional[str] = None) -> Optional[List[Dict[str, Any]]]:
        ...
        Returns:
            類似画像のリスト（SDS_FILESとSDS_IMAGE_EMBEDDINGSをJOINした結果）
        """
        try:
            if not self._ensure_pool_initialized():
                logger.error("接続プールの初期化に失敗")
            ...
            with self._get_pool_manager().acquire_connection() as connection:
                ...
                with connection.cursor() as cursor:
                    # SDS_FILESとSDS_IMAGE_EMBEDDINGSをJOINしてベクトル検索
                    ...
                    FROM 
                        SDS_IMAGE_EMBEDDINGS ie
                    INNER JOIN 
                        SDS_FILES f ON ie.FILE_ID = f.FILE_ID
                    WHERE 
                        {where_clause}
                    ORDER BY 
                        vector_distance
