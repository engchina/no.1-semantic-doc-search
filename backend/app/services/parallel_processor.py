...
ページ画像化（ProcessPool）とベクトル化（ThreadPool）を管理するサービスです。
CPU集中型処理とI/O集中型処理を適切に並列化し、システムリソースを効率的に利用します。
...
- API呼び出しの同時実行数制限（セマフォ）

設計原則:
- CPU負荷の高い画像変換処理はプロセスプールで実行
- I/O待ちが発生するベクトル化処理はスレッドプールで実行
- 各処理単位は独立して実行可能（スレッドセーフ）
"""
import asyncio
import io
import logging
