...
test('AG-UI検索イベントは進捗を表示して結果を描画する', async () => {
  ...
  const stream = [
    ...
    { type: 'STATE_DELTA', delta: [{ op: 'replace', path: '/queryPlan', value: { variants: ['天井照明', 'ダウンライト'], intent: 'lighting', keyword_query: '天井照明 ダウンライト' } }] },
  ...
  assert.match(document.getElementById('searchAgentDetails').textContent, /ダウンライト/);
