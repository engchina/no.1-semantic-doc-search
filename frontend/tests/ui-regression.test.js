...
test('新しい共有検索の有効状態でも最小スコアを有効のままにする', async () => {
  document.body.innerHTML = `
    <fieldset id="dynamicSearchFilters" hidden><div id="dynamicSearchFilterFields"></div></fieldset>
    <label id="minScoreLabel" for="minScore">最小スコア</label><input id="minScore" value="0.55">
  ...
  assert.equal(document.getElementById('minScoreLabel').textContent, '最小スコア');
...
test('検索方式が未選択なら送信せずインラインエラーを表示する', async () => {
  document.body.innerHTML = `
    ...
    <input id="filenameFilter" value=""><input id="topK" value="10"><input id="minScore" value="0.55">
...
test('AG-UI検索イベントは進捗を表示して結果を描画する', async () => {
  ...
  assert.equal(requestBody.min_score, 0.55);
...
test('画像検索の分割SSEはheartbeatを無視して段階進捗を描画する', async () => {
  ...
  assert.equal(requestBody.get('min_score'), '0.55');
